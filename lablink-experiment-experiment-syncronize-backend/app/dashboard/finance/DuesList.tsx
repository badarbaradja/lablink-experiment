...
import { Check, RefreshCw, Eye, XCircle } from 'lucide-react';
...
export default function DuesList() {
  ...
  const handleReject = async (id: string) => {
    if (!confirm('Tolak pembayaran ini? Bukti bayar dianggap tidak valid.')) return;
    try {
      setProcessingId(id);
      await api.post(`/finance/dues/${id}/reject`, {});
      showSuccess('Pembayaran ditolak');
      fetchDues();
    } catch (err) {
      showError('Gagal menolak pembayaran');
    } finally {
      setProcessingId(null);
    }
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      header: 'Status',
      render: (item: DuesPayment) => {
        // PERBAIKAN ERROR TS2367: Menggunakan pengecekan yang lebih aman terhadap tipe data
        let config = { label: 'Menunggu', class: 'bg-yellow-100 text-yellow-800' };
        
        const statusString = String(item.status);
        
        if (statusString === 'VERIFIED' || statusString === 'PAID') {
          config = { label: 'Lunas', class: 'bg-green-100 text-green-800' };
        } else if (statusString === 'REJECTED') {
          config = { label: 'Ditolak', class: 'bg-red-100 text-red-800' };
        } else if (statusString === 'PENDING') {
          config = { label: 'Menunggu Verifikasi', class: 'bg-yellow-100 text-yellow-800' };
        }
      ...
      render: (item: DuesPayment) => {
        ...
        return (
          <div className="flex gap-2">
            ...
            <Button 
                size="sm" 
                className="bg-red-600 hover:bg-red-700 h-8 w-8 p-0 flex items-center justify-center text-white"
      ...
      {selectedProof && (
         ...
         >
            <div className="flex justify-center">
                {/* PERBAIKAN TAILWIND: Menggunakan max-h-125 sesuai saran compiler */}
                <img src={selectedProof} alt="Bukti Transfer" className="max-h-125 object-contain rounded-lg border" />
            </div>
